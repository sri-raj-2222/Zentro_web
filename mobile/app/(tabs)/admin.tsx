...
type AdminTab = "overview" | "attendance" | "feedback";
...
export default function AdminScreen() {
  ...
  useEffect(() => {
    ...
    const feedbackChannel = supabase
      .channel(`public:feedbacks_realtime_${Math.random().toString(36).slice(2, 7)}`)
      .on(
        "postgres_changes",
    ...
    const profileChannel = supabase
      .channel(`public:profiles_${Math.random().toString(36).slice(2, 7)}`)
      .on(
        "postgres_changes",
      ...
      {activeTab === "overview" && (
        ...
        >
          ...
          {workerList.map((w) => {
            ...
            const wStatus = workerStatuses[w.id]?.status || "available";
            return (
