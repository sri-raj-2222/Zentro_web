...
export default function AttendancePage() {
  ...
  const { workerStatuses, bookings } = useBookings();
  const {
    attendance,
    markAttendance,
    updateCommission,
  ...
  useEffect(() => {
    if (user) {
      ...
      } else if (workersList.length > 0 && !selectedWorkerId) {
        setSelectedWorkerId(workersList[0].id);
      }
    }
  }, [user, workerStatuses, selectedWorkerId]);
  ...
  if (authLoading || !user || (user.role !== "admin" && user.role !== "worker")) {
    return (
  ...
  const selectedWorker = workersList.find((w) => w.id === selectedWorkerId) || (user.role === "worker" ? { id: user.id, name: user.name, average_rating: 5, status: "available" as const, total_feedbacks: 0 } : null);
  ...
  const presentDaysCount = monthlyRecords.filter((r) => r.status === "present").length;
  const absentDaysCount = monthlyRecords.filter((r) => r.status === "absent").length;
  const halfDaysCount = monthlyRecords.filter((r) => r.status === "half_day").length;
  const holidayDaysCount = monthlyRecords.filter((r) => r.status === "holiday").length;

  // Build a map: dateStr -> count of completed jobs for selected worker on that day
  const jobsPerDay: Record<string, number> = {};
  if (selectedWorkerId) {
    bookings
      .filter(
        (b) =>
          b.workerId === selectedWorkerId &&
          b.status === "completed" &&
          (b.scheduledDate || b.updatedAt)
      )
      .forEach((b) => {
        const raw = b.scheduledDate || b.updatedAt;
        const dateKey = raw.slice(0, 10); // "YYYY-MM-DD"
        jobsPerDay[dateKey] = (jobsPerDay[dateKey] || 0) + 1;
      });
  }
  ...
  return (
    <div className={styles.container}>
        ...
        <div className={styles.headerTitleWrap}>
          ...
          <div>
            <h1 className={styles.title}>
              {user.role === "worker" ? "My Attendance" : "Workers & Commissions"}
            </h1>
            <p className={styles.subtitle}>
              {user.role === "worker"
                ? "View your shifts, attendance statistics, and estimated payout."
                : "Manage worker shifts, mark daily sheets, and adjust payroll commission rates."}
            </p>
          </div>
        </div>
        {user.role === "admin" && (
      ...
      {user.role === "worker" ? (
        ...
        <div className={styles.workerDashboardGrid}>
          {/* Top selection & stats row */}
          <div className={styles.row}>
            ...
            <div className={styles.card} style={{ flex: 1 }}>
              ...
              <div className={styles.monthSelectWrap} style={{ display: "block" }}>
                <select
                  value={selectedMonth}
              ...
              <div className={styles.monthsGrid} style={{ display: "grid" }}>
                {MONTHS.map((m, index) => {
            ...
            {/* MONTH STATS CARD */}
            <div className={`${styles.card} ${styles.flexCard}`} style={{ flex: 2 }}>
              <h2 className={styles.cardTitle}>
                <TrendingUp size={18} />
              ...
              <div className={styles.metricsGrid}>
                <div className={styles.metricItem} style={{ borderLeft: "3.5px solid #22c55e" }}>
                  <span className={styles.metricValue}>{presentDaysCount}</span>
                  <span className={styles.metricLabel}>Present Days</span>
                </div>
                <div className={styles.metricItem} style={{ borderLeft: "3.5px solid #ef4444" }}>
                  <span className={styles.metricValue}>{absentDaysCount}</span>
                  <span className={styles.metricLabel}>Absent Days</span>
                </div>
                <div className={styles.metricItem} style={{ borderLeft: "3.5px solid #f59e0b" }}>
                  <span className={styles.metricValue}>{halfDaysCount}</span>
                  <span className={styles.metricLabel}>Half Days</span>
                </div>
                <div className={styles.metricItem} style={{ borderLeft: "3.5px solid #3b82f6" }}>
                  <span className={styles.metricValue}>{holidayDaysCount}</span>
                  <span className={styles.metricLabel}>Holidays</span>
                </div>
              </div>
              
              <div className={styles.metricsGrid} style={{ marginTop: 12 }}>
                ...
                <div className={styles.metricItem}>
                  ...
                  <span className={styles.metricLabel}>Gross Bookings</span>
                </div>
                <div className={styles.metricItem}>
                  <span className={styles.metricValue}>{salaryMetrics.commissionPercentage}%</span>
                  <span className={styles.metricLabel}>My Cut Rate</span>
                </div>
              </div>
              
              <div className={styles.earningsSummaryBox} style={{ marginTop: 16 }}>
                <div>
                  <span className={styles.payoutLabel}>Estimated Net Payout</span>
                  <span className={styles.payoutRate}>Based on completed services</span>
                </div>
          ...
          <div className={styles.card}>
            <div className={styles.calendarHeader}>
              <h2 className={styles.cardTitle} style={{ margin: 0 }}>
                <Calendar size={18} />
                <span>My Shift Calendar ({MONTHS[selectedMonth - 1]})</span>
              </h2>
              <span className={styles.calendarTargetName}>
                All shifts are marked by administrator
              </span>
            </div>
            
            <div className={styles.calendarGrid} style={{ marginTop: 20 }}>
              {Array.from({ length: daysInMonth }, (_, index) => {
                ...
                const isUpcoming = cellDate > today;
                ...
                return (
                  <div key={dayNum} className={styles.calCellWrapper}>
                    <div
                      className={`${styles.calendarDayCell} ${statusConfig && !isUpcoming ? "" : styles.unmarkedCell} ${isUpcoming ? styles.calendarDayCellDisabled : ""}`}
                      style={
                        statusConfig && !isUpcoming
                          ? { border: `1.5px solid ${statusConfig.color}`, backgroundColor: `${statusConfig.bgColor}50`, cursor: "default" } 
                          : { cursor: "default" }
                      }
                    >
                      ...
                      {jobCount > 0 && !isUpcoming && (
                        <span className={styles.jobCountBadge} title={`${jobCount} job${jobCount > 1 ? "s" : ""} completed`}>
                      ...
                      return (
                        <div key={dayNum} className={styles.calCellWrapper}>
                          ...
                          >
                            ...
                            {(() => {
                              const jc = jobsPerDay[dateStr] || 0;
                              return jc > 0 && !isDisabled ? (
                                <span className={styles.jobCountBadge} title={`${jc} job${jc > 1 ? "s" : ""} completed`}>
                                  {jc} job{jc > 1 ? "s" : ""}
                                </span>
                              ) : null;
                            })()}
