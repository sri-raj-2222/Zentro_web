...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`user_feedback_${user.id}`)
      .on(
        "postgres_changes",
        { 
          event: "UPDATE", 
        ...
        (payload) => {
          ...
          if (payload.new.status === "completed") {
            refreshBookings().then(() => checkFeedback());
          }
        }
      )
      .subscribe();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <ServiceCard image={require("@/assets/images/car_service.png")} title="🚗 Car Wash" subtitle="Full exterior & interior cleaning" price={getPrice("car_wash").toString()} color="#dc2626" onPress={() => router.push({ pathname: "/book", params: { service: "car_wash" } })} />
        <ServiceCard image={require("@/assets/images/bike_service.png")} title="🏍️ Bike Wash" subtitle="Thorough bike cleaning & polishing" price={getPrice("bike_wash").toString()} color="#7c3aed" onPress={() => router.push({ pathname: "/book", params: { service: "bike_wash" } })} />
        <ServiceCard image={require("@/assets/images/tank_service.png")} title="💧 Water Tank Cleaning" subtitle="Deep tank cleaning & sanitization" price={getPrice("water_tank").toString()} color="#059669" onPress={() => router.push({ pathname: "/book", params: { service: "water_tank" } })} />
